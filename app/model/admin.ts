...
module.exports = app => {
    ...
    class Admin extends BaseModel {
         ...
         * @return {Object} Promise
         */
        async ownedMaterials() {
            const self = this;
            const mappings = await app.model.AdminMaterialMapping.findAll({
                where: {
                    adminId: self.id,
                }
            });
            const materials = await Promise.map(mappings, async item => {
                return app.model.Material.findById(item.materialId);
            });
            return materials;
        }
        /**
         ...
         * @return {Object} Promise
         */
        async ownedMaterialStatistics(ctx, id, materialId) {
            const self = this;
            const mapping = await app.model.AdminMaterialMapping.findOne({
                where: {
                    adminId: self.id,
                    materialId,
                }
            });
            const statistices = await app.model.Statistic.findAll({
                where: {
